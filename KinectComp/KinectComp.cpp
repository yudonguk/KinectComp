...
ReturnType KinectComp::onInitialize()
{
	...
	//API Releaser 가져오기
	mKinectApiReleaser = (void(*)(OprosApi*))GetProcAddress(mhOprosAPI, "ReleaseAPI");
	if(mKinectApiReleaser == NULL)
	{
		PrintMessage(DEBUG_MESSAGE("Can't get a handle of ReleaseAPI Funtion").c_str());
	}
...
{
	vector<unsigned long> &userID = GetSkeletonID();
	...
	{
		vector<unsigned long> deletedID;
		vector<unsigned long> addedID;
...
vector<unsigned long> KinectComp::GetSkeletonID()
{
	if (mpKinect == NULL)
	{
		PrintMessage(DEBUG_MESSAGE("Can't get a handle of Kinect API").c_str());
	...
	vector<unsigned long> skeletonID;
...
vector<Skeleton> KinectComp::GetSkeletonAll()
{
	if (mpKinect == NULL)
	{
		PrintMessage(DEBUG_MESSAGE("Can't get a handle of Kinect API").c_str());
		return vector<Skeleton>();
	}
...
Property KinectComp::GetParameter()
{	
	if (mpKinect == NULL)
	{
		PrintMessage(DEBUG_MESSAGE("Can't get a handle of Kinect API").c_str());
	...
	Property parameter;
	if (mpKinect->GetParameter(parameter) != API_SUCCESS)
	{
...
bool KinectComp::SetParameter(Property parameter)
{
	if (mpKinect == NULL)
	{
		PrintMessage(DEBUG_MESSAGE("Can't get a handle of Kinect API").c_str());
	...
	if (mpKinect->SetParameter(parameter) != API_SUCCESS)
	{
